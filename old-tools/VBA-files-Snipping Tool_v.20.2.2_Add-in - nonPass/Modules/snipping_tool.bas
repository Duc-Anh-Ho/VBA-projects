Attribute VB_Name = "snipping_tool"
Option Explicit
...
no_object:
    If Err.Number = 91 Then
        ...
        Exit Sub
    Else
        MsgBox "Error Code" & Err.Number, vbOKOnly + vbCritical
    End If
    
'Option_1.1 selected range
    'Range setup
    ...
    '3.2.Set size (if not move with cell)
        With pic_captured_screen
            ...
            End If
        End With
'Option_1.2 selected DrawingObjects (Group)
    ElseIf TypeName(Selection) = "DrawingObjects" Then
        MsgBox "Sorry! we do not support " & TypeName(Selection) & " yet" _
            ...
            "SNIPPING TOOL"
'Option_1.3 selected object (picture, shape, chart, ...)
    Else
        'Confirm notification
        Dim replace_confirm As Byte
